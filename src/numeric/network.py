...
class Network:   
    ...
    def compute_output(self, input):
        activation = input
        for i, (layer_biases, layer_weights) in  enumerate(zip(self.parameters["biases"], self.parameters["weights"])):
            preactivation = np.dot(activation, layer_weights)*1.0/np.sqrt(layer_weights.shape[0])+layer_biases           
        ...
        for i, layer_weights in enumerate(weights):
            ...
            for j, weight_row in enumerate(layer_weights):
                for k, weight_column in enumerate(weight_row):                 
                    ...
                    weights[i][j][k] -= delta
                    gradient = (adjusted_loss- original_loss)/delta
                    new_layer_weights[j][k] = weights[i][j][k]- learning_rate*gradient
    ...
    def compute_loss(self, training_data):
        return sum([ self.loss_function(self.compute_output(training_input), training_output) for training_input, training_output in zip(*training_data)])

    def loss_function(self, actual, target):
        return loss_functions[self.hyperparameters["training"]["loss_function"]](actual, target)    
    
    def activation_function(self, preactivation):
        return activation_functions[self.hyperparameters["activation_function"]](preactivation)
    
    def output_activation_function(self, output_preactivation):
        return activation_functions[self.hyperparameters["output_activation_function"]](output_preactivation)
