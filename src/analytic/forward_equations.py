...
def calculate_Theta(x, rho, lambda_b, lambda_w, C_b, C_w, n_0, layer):
    ...
    if(layer == 1):        
        ...
        return (lambda_b + 
                ...
                +C_w*calculate_Theta(x, rho, lambda_b, lambda_w, C_b, C_w, n_0, layer-1))    
...
def calculate_K(x, rho, C_b, C_w, n_0, layer):
    ...
    if(layer == 1):        
        ...
        return (C_b + 
                C_w*correlator(rho, calculate_K(x, rho, C_b, C_w, n_0, layer -1)))    
    ...
    for (delta_1, delta_2) in np.ndindex(K.shape):                 
        if(delta_1 == delta_2):            
            correlator[delta_1][delta_2] = sp.integrate.quad(
                lambda phi: rho(phi)**2 *math.exp(- 0.5 * phi**2 / K[delta_1][delta_1])/ math.sqrt(2 * math.pi * K[delta_1][delta_1]),
                -math.inf,
                +math.inf
                )[0]
        else:           
                      
            K_reduced = np.array([[K[delta_1][delta_1],K[delta_1][delta_2]],
            ...
            def integrand(phi_delta_1, phi_delta_2, K_reduced_inv):
                ...
                return rho(phi_delta_1)*rho(phi_delta_2) *math.exp(- 0.5 * phi.T @ K_reduced_inv @ phi)
            ...
            correlator[delta_1][delta_2] = integral/(2 * math.pi * math.sqrt(K_reduced_det))
