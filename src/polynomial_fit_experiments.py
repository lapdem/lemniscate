...
for order in range(highest_order):
    experiment = ex.Experiment(
        input_folder, configuration_file_name, output_folder + str(order)
    )
    original_x, original_y = experiment.data
    coefficents = np.polynomial.polynomial.polyfit(
        np.ndarray.flatten(original_x), np.ndarray.flatten(original_y), order
    )
